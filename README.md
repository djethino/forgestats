...
- **Shareable URLs.** `#main=github:user&mirrors=sourceforge:user,codeberg:user` — send your stats page to anyone. A repository URL as input (`github.com/user/repo`) jumps straight to that repo's page. Manual links are remembered in your browser (per main profile, shared by all open tabs); *Copy configuration link* adds them to the URL (`&links=repo>sourceforge:project/folder`), and opening such a link stores them.
